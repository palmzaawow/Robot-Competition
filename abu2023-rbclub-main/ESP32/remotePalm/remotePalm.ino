...
#include <WiFi.h>
#include "esp_now.h"
uint8_t broadcastAddress[] = {0x0C, 0xB8, 0x15, 0xC2, 0x42, 0x60};
...
void OnDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len) {
  ...
  //Serial.println(millis()-myData.prevCycleMillis);
...
void setup() {
  ...
  pinMode(enleftB, INPUT_PULLUP);
  pinMode(enrightA, INPUT_PULLUP);
  pinMode(enrightB, INPUT_PULLUP);
  ...
  WiFi.disconnect();
      // Serial.begin(115200);
  /*   WiFi.begin(ssid, password);  //dfdsfsd
