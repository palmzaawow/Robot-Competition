...
void QuadMotorDriver::setPos(uint8_t number,int64_t position){
  M[number - 1].positionControl.targetedPos = position;
}

int64_t QuadMotorDriver::getPos(uint8_t number){
  return M[number-1].positionControl.targetedPos;
}
